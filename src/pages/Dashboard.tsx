...
export default function Dashboard() {
  ...
  const getWelcomeMessage = () => {
    switch (user?.role) {
      case 'admin': return { title: `Founder Command Center`, sub: 'Real-time visibility into operational execution and financials.' };
      default: return { title: `Welcome to HireNest`, sub: 'Enterprise Relationship Management' };
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col gap-6">
          <div className="flex flex-col">
             ...
             <p className="text-sm text-slate-500">Pipeline volume from CRM and OS</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
             <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
               <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Open</p>
               <p className="text-3xl font-black text-slate-900">{openRequirements}</p>
             </div>
             <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
               <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Closed</p>
               <p className="text-3xl font-black text-slate-900">{closedRequirements}</p>
             </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col gap-6">
          <div className="flex flex-col">
             ...
             <p className="text-sm text-slate-500">Candidate flow through delivery</p>
          </div>
          <div className="grid grid-cols-3 gap-3">
             <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex flex-col items-center justify-center">
               <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1">Subs</p>
               <p className="text-2xl font-black text-slate-900">{totalSubmissions}</p>
             </div>
             <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex flex-col items-center justify-center">
               <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1">Intvws</p>
               <p className="text-2xl font-black text-slate-900">{interviewsScheduled}</p>
             </div>
             <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex flex-col items-center justify-center">
               <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1">Offers</p>
               <p className="text-2xl font-black text-slate-900">{offersReleased}</p>
             </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-emerald-500 to-emerald-700 p-6 rounded-3xl border border-emerald-600 shadow-md flex flex-col gap-6 text-white overflow-hidden relative group">
          ...
          <div className="flex flex-col relative z-10">
             ...
             <p className="text-sm text-emerald-100">Closed deals and joined candidates</p>
          </div>
          <div className="flex items-end justify-between relative z-10 mt-auto">
             <div>
               <p className="text-xs font-black uppercase tracking-widest text-emerald-200 mb-1">Total</p>
      ...
      <div className="bg-slate-900 rounded-[2rem] p-8 lg:p-10 border border-slate-800 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-indigo-400 font-black uppercase tracking-widest text-xs mb-8 flex items-center gap-2">
            <ShieldCheck className="w-4 h-4" /> CRM Commercials
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-sm">
               <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Expected Revenue</p>
               <p className="text-3xl font-black text-white">{formatCurrency(expectedRevenue)}</p>
            </div>
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-sm">
               <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Collections Due</p>
               <p className="text-3xl font-black text-orange-400">{collectionsDue === 0 ? '₹0' : formatCurrency(collectionsDue)}</p>
            </div>
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-sm">
               <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Vendor Payables</p>
               <p className="text-3xl font-black text-rose-400">{vendorPayables === 0 ? '₹0' : formatCurrency(vendorPayables)}</p>
            </div>
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-sm">
               <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Actual Margin</p>
               <p className="text-3xl font-black text-emerald-400">{actualMargin === 0 ? '₹0' : formatCurrency(actualMargin)}</p>
            </div>
          </div>
        </div>
      </div>
